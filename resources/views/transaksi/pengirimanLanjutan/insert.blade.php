			...
			<div class="block">
				<div class="block-header block-header-default">
					<h3 class="block-title">
						<a class="btn btn-warning btn-sm float-right" href="{{ route('manifest.insert') }}"> <i class="fa fa-chevron-left"></i> Back</a>
					</h3>
				</div>
				<div class="block-content block-content-full">									
					<?php if($set!="generate"){ $rt = ""; ?>
						<form class="mb-5" action="{{ route('manifest.generate') }}" enctype="multipart/form-data" method="POST">					
					<?php }else{ $rt = "readonly"; ?>
						<form class="mb-5" action="{{ route('manifest.create') }}" enctype="multipart/form-data" method="POST">					
					<?php } ?>
						...
						<div class="form-group row">														
							...
							<div class="col-sm-2">
								<input type="date" <?=$rt?> class="form-control" name="tgl_manifest" value="{{ $tgl_manifest }}" placeholder="Tanggal Manifest">
							</div>																			
							<label class="col-sm-2 text-right col-form-label" >Estimasi (hari)</label>
							<div class="col-sm-1">
								<input type="number" name="estimasi" class="form-control" value="{{ $estimasi }}">	
							</div>	
						</div>
						<div class="form-group row">																														
							<label class="col-sm-2 col-form-label text-right">Port Asal</label>
							<div class="col-sm-3">
								<select <?=$rt?> class="form-control" id="port_asal" name="port_asal" required>
									<option value="">- choose -</option>
									@foreach ($cabang as $key => $dt)										
										@php
										if($port_asal!='' && $port_asal==$dt->id) $sel = 'selected';
											else $sel = '';
										@endphp
							...
							<div class="col-sm-3">
								<select <?=$rt?> class="form-control" id="port_tujuan" name="port_tujuan" required>
									<option value="">- choose -</option>
									@foreach ($cabang as $key => $dt)										
										@php
										if($port_tujuan!='' && $port_tujuan==$dt->id) $sel = 'selected';
											else $sel = '';
										@endphp
										<option <?=$sel?> value="{{ $dt->id }}">{{ $dt->name }} </option>
									@endforeach
						...
						<div class="form-group row">						
							...
							<div class="col-sm-4">
								...
								<a href="{{ route('manifest.insert') }}" class="btn btn-secondary btn-sm"> Reset</a>
							</div>
							<?php } ?>
						</div>					
				</div>
