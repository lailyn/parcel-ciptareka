				...
				<div class="block-content block-content-full">
					        	...
					        	@foreach ($manifestMentah as $key => $isi)
										...
										<tr>
											...
											<td class="text-center">
												<a href="{{ route('manifestUtama.addItem',[$isi->id,$row->code]) }}" class="btn btn-success btn-sm">
													pilih
												</a>
											</td>
										</tr>
					...
					<table class="table table-bordered table-striped table-vcenter">
						...
						@foreach ($manifestDetail as $key => $dt)
							...
							$asal = ($cariAsal->count())?$cariAsal->first()->name:'';
							$cekTotal = DB::table("manifestUtama_detail")->where("code",$dt->code)->count();							
							$status = "<span class='badge badge-info'>$dt->status</span>";
							?>
							<tr>
								<th class="text-center">{{ $key+1 }}</th>								
								...
								<td>{!! $status !!}</td>
								<td class="text-center">
									<a href="{{ route('manifestUtama.deleteDetail', $dt->ids) }}" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure...?')">Delete</a>
								</td>
							</tr>
						@endforeach													
						</tbody>
